...
public abstract class ActiveRouter extends MessageRouter {
	...
	public int receiveMessage(Message m, DTNHost from) {
                //System.out.println("At: "+ this.getHost().toString() +" From: "+ from.toString()+" Received: "+m.toString());
            //Added for prob and scope -------------------------------
                double rand = Math.random();
                if(rand > 1){ // prob to process
                    //System.out.print(rand+"\n");
                    return DENIED_POLICY;
                    
                }           
                
                if(m.toString().contains("request") || m.toString().contains("response")){
                    if(m.getHopCount() > 3){
                        //System.out.println(SimClock.getTime()+" Host: "+ getHost()+" "+m.toString()+" denied; max hop!"+m.getHopCount());
                        return DENIED_NO_SPACE;
                    }
                }
                if(m.toString().contains("ping")){
                    if(m.getHopCount()> 3){
                        //System.out.println(SimClock.getTime()+" Host: "+getHost()+" "+m.toString()+" denied; max hop!"+m.getHopCount());
                        return DENIED_NO_SPACE;
                    }
                ...
                if(!getHost().getNs().messageMatchesInterest(m, 0, false)){
                    System.out.println(SimClock.getTime()+" Host: "+getHost()+" not interested in "+m.toString()+" denied at level 0!");
                    return DENIED_NO_SPACE;
                }
                else if(getHost().getNs().messageMatchesInterest(m, 0, false) && getHost().toString().startsWith("i")){
                    if(!getHost().getNs().messageMatchesInterest(m, 0, true)){
                        System.out.println(SimClock.getTime()+" Host: "+getHost()+"\t"+m.toString()+" irrelevant but not denied at level 0!");
                    }
                }
            //-------------------------------------------------Added for prob and scope    
