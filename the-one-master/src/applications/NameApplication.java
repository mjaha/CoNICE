...
public class NameApplication extends Application {
	 ...
	 */
	public NameApplication(Settings s) {
	 ...
	 * @param a
	 */
	public NameApplication(NameApplication a) {
		super(a);
...
//                        System.out.println("\t***This is sent to me!");
	...
	public Application replicate() {
		return new NameApplication(this);
	}
	...
	public void update(DTNHost host) {
		...
		double curTime = SimClock.getTime();
                if(curTime == endSimTime){// print final logs
                    System.out.println("$FIN\t"+host+"\t"+mapSize()+"\t"+appliedMessageSet.size());
                }
		...
		if (curTime - this.lastPing >= this.interval && curTime <= endSendTime) {
			...
			host.createNewMessage(m);
                        ...
                        if(host.toString().startsWith(NAMESPACE_MANAGER_PREFIX)){//add my own
                            System.out.print("\t\tcreated "+ m.toString()+"\n");
...
//
            ...
            System.out.println("\t"+mapSize()+"\t"+allMessages);
        ...
        public void addToAppliedMessageSet(List<Message> mList){
            int index=1;
            int flag=0;
            int flagExp=0;
            while(true){
                            ...
                            if(allRefsApplied == true){
                                ...
                                if(m1RefForS!=null){
                                    for(String m1RefFor: m1RefForS){
                                        ...
                                        if(m11List!=null){
                                            addToAppliedMessageSet(m11List);
                                        }
                                    }
                                }
                            }
